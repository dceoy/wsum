...
record, and a `recovery_action`. For `release_target_claim`, it also returns a
complete `release_request`; pass that object unchanged to
`local-notification-release`. For an unpromoted delivered candidate it also
...
`local-snapshot-promote`, then pass the returned `release_request` unchanged to
`local-notification-release`. Release a pending failure, promote an unpromoted
